...
def train(
    ...
    # progress.add_task(description="Initialising Optimizer ...", completed=3, total=4)
    optimizer = model.get_optimizer(model_lr=mlr if not lr_find else 1,
                                    model_wd=mwd,
    ...
    eval_dl = training_data.eval_ds.get_dataloader(batch_size=bs,
                                            seq_len=model.config.max_seq_len,
                                            batch_by_token_count=True,
                                            pin_memory=True)
    ...
    trainer.info(f"Hparams: {json.dumps(hparams, indent=4)}")
    trainer.info(f"Training Examples: {len(training_data.train_ds)}")
    trainer.info(f"Evaluation Examples: {len(training_data.eval_ds)}")
