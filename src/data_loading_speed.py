...
from src.task import TRAIN_ONLY_COLLECTION, ArcTrainingDataset, Example
from collections import defaultdict
#%%

if __name__ == "__main__":

    training_data: ArcTrainingDataset = TRAIN_ONLY_COLLECTION
    ...
    print(f"Time taken to filter: {(time.time() - start_time)*1000} ms")

    #%%
    start_time = time.time()


    training_data.augment(min_test=1,
                        max_test=3,
                        max_train=None,
                        min_train=100)
    training_data.stats()
